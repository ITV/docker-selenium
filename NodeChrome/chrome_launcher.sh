...
exec -a "$0" "$HERE/chrome" --disable-impl-side-painting --ignore-certifcate-errors --no-sandbox "$PROFILE_DIRECTORY_FLAG" \
  "$@"
exec -a "$0" /etc/alternatives/google-chrome --disable-impl-side-painting --ignore-certifcate-errors --no-sandbox "$@"
